...
| Git update via `fetch` + `reset --hard` (never `ansible.builtin.git` `force: true`) | `force: true` in ansible git module runs `git clean -fdx` — would delete `data/`, `backups/`, logs. `fetch`+`reset` only touches tracked files |
...
# Caddy runtime dirs (generated by Docker, never in the app image)
caddy_data/
caddy_config/
caddy_logs/
...
- name: Deploy radtracker to VPS
  ...
  tasks:
    ...
    - name: Clone or reset repository (preserves untracked files)
      ansible.builtin.shell:
        cmd: |
          if [ -d "{{ radtracker_dir }}/.git" ]; then
            cd "{{ radtracker_dir }}"
...
**Dependency:** Requires `community.docker` collection. Install once:
```bash
...
- name: Check radtracker health
  ...
  tasks:
    ...
    - name: Check Caddy is serving (expect 401 — no auth provided)
      ansible.builtin.uri:
        url: "http://localhost/"
        status_code: 401
      register: caddy_endpoint

    - name: Assert Caddy responds with 401
      ansible.builtin.assert:
        that: caddy_endpoint.status == 401
        fail_msg: "Caddy returned {{ caddy_endpoint.status }} (expected 401)"
        success_msg: "Caddy responding (401 — auth required)"
